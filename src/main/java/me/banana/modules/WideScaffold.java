...
public class WideScaffold extends Module {
    ...
    public WideScaffold() {
        super(BananaHack.BANANAHACK, "wide-scaffold", "Scaffold but wider.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (!renders.isEmpty()) {
            renders.forEach(Renderers.SimpleBlockRender::tick);
            renders.removeIf(Renderers.SimpleBlockRender::shouldRemove);
        }
        ...
        for (BlockPos pos : nearby) {
            ...
            renders.add(new Renderers.SimpleBlockRender(pos, renderT, sideColor.get(), lineColor.get(), fadeFactor.get()));
        }
    }
