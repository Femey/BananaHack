...
import me.banana.util.CrystalUtils;
import me.banana.util.EntityUtil;
import me.banana.util.PlayerUtil;
import me.banana.util.TimerUtils;
...
public class CrystalAuraPlus extends Module {
    ...
    public CrystalAuraPlus() {
        super(BananaHack.BANANAHACK, "CrystalAura+", "Automatically places and attacks crystals.");
    }
    ...
    private float getBreakDamage(Entity entity, boolean checkCrystalAge) {
        ...
        if (!CrystalUtils.shouldIgnoreSelfBreakDamage()) {
            float selfDamage = EntityUtil.crystalDamage(mc.player, entity.getPos(), predictMovement.get(), breakRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            if (!CrystalUtils.shouldIgnoreSelfPlaceDamage()) {
                float selfDamage = EntityUtil.crystalDamage(mc.player, vec3d, predictMovement.get(), placeRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private void placeCrystal(BlockHitResult result, double damage, BlockPos supportBlock) {
        ...
        if (supportBlock == null) {
            ...
            if (renderBlockPool.get().pos != result.getBlockPos()){
    ...
    private float getDamageToTargets(Vec3d vec3d, boolean breaking, boolean fast) {
        ...
        if (fast) {
            ...
            if (!(smartDelay.get() && breaking && target.hurtTime > 0)) damage = EntityUtil.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
        }
        else {
            for (PlayerEntity target : targets) {
                if (smartDelay.get() && breaking && target.hurtTime > 0) continue;

                float dmg = EntityUtil.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        Vec3 vec = new Vec3(renderPos.getX() + 0.5, renderPos.getY() + 0.5, renderPos.getZ() + 0.5);
