...
import me.banana.util.TimerUtil;
...
public class StrafePlus extends Module {
    ...
    public StrafePlus() {
        super(BananaHack.BANANAHACK, "Strafe+", "Straight from banana+");
    }
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (EntityUtil.isWebbed(mc.player) && webbedPause.get() == WebbedPause.Always) return;
        if (EntityUtil.isWebbed(mc.player) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;

        if (mc.player.isOnGround()) {
           timerClass.setOverride(PlayerUtils.isMoving() ? (groundTimer.get() * TimerUtil.TPSSync(TPSSync.get())) : Timer.OFF);
        } else
           timerClass.setOverride(PlayerUtils.isMoving() ? (airTimer.get() * TimerUtil.TPSSync(TPSSync.get())) : Timer.OFF);
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (WorldUtil.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (WorldUtil.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
